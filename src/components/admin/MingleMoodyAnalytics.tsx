...
import { Users, Eye, MousePointer, Search, Link, Monitor, Smartphone, ChevronDown, ChevronUp, Calendar, RefreshCw } from "lucide-react";
...
export const MingleMoodyAnalytics = () => {
  ...
  const getCountryFlag = (country: string | null) => {
    if (!country || country === 'WW' || country === 'Unknown') return '🌍';
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-zinc-900 border-zinc-800">
        <CardHeader className="bg-cyan-500/10 border-b border-zinc-800">
          <CardTitle className="text-cyan-400">Session Analytics ({filteredSessions.length} sessions)</CardTitle>
        </CardHeader>
                ...
                {filteredSessions.map((session) => (
                  <tr key={session.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                    ...
                    <td className="p-3 text-zinc-400 text-sm">
                      <div>
                        <div>{session.last_activity ? new Date(session.last_activity).toLocaleDateString() : '-'}</div>
                        <div className="text-[10px]">{session.last_activity ? new Date(session.last_activity).toLocaleTimeString() : ''}</div>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <Dialog open={showSearchBreakdown} onOpenChange={setShowSearchBreakdown}>
        <DialogContent className="bg-zinc-900 border-zinc-800 max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-cyan-400">Click Breakdown: {selectedSearchName}</DialogTitle>
          </DialogHeader>
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {clickDetails.map((click) => (
              <div key={click.id} className="flex items-center gap-4 p-2 bg-zinc-800 rounded border border-zinc-700">
            ...
            {clickDetails.length === 0 && (
              <p className="text-zinc-500 text-center py-4">No click details found</p>
            )}
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showTotalClicksBreakdown} onOpenChange={setShowTotalClicksBreakdown}>
        <DialogContent className="bg-zinc-900 border-zinc-800 max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-cyan-400">All Clicks Breakdown</DialogTitle>
          </DialogHeader>
          <div className="space-y-2 max-h-96 overflow-y-auto">
            ...
            {allClicksData.length === 0 && (
              <p className="text-zinc-500 text-center py-4">No click data found</p>
            )}
          </div>
        </DialogContent>
      </Dialog>
