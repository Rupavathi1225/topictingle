...
const DataCreditZoneBlogPost = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: blogData, error } = await tejaStarinClient
        .from("blogs")
        .select("*")
        .eq("slug", blogSlug)
        .eq("status", "published")
      ...
      const { data: searches } = await tejaStarinClient
      ...
      if (blogData.category_id) {
        const { data: categoryData } = await tejaStarinClient
          .from("categories")
          .select("id, name, slug")
        ...
        const { data: recent } = await tejaStarinClient
          .from("blogs")
          .select("*")
          .eq("status", "published")
                    ...
                    <div className="space-y-4">
                      {recentPosts.map((post) => (
                        <div
                          ...
                          onClick={() =>
                            (window.location.href = `/datacreditzone/blog/${category?.slug}/${post.slug}`)
                          }
                        >
...
export default DataCreditZoneBlogPost;
